# human-mouse variant correspondence (2026 rebuild)

2025年版の仕様を基に、最新データの取得、整形、3つの独立した対応戦略に変更して、再実装

## 概要

ヒトvariantとマウスalleleの対応の候補を、単一の「正解」ではなく、**どの情報で接続されたか**とともに扱うためのパイプライン。
| 解説資料の項目 |　リンク |
| --- | --- |
| 今回の開発の概要 | このREADME |
| Tier 1/2/3が何を根拠にアレルを接続するか | [`output/tier_evidence_explainer.html`](output/tier_evidence_explainer.html) |
| TLR4・CFTR・ALPLによる検証 | [`output/case_study_evidence_explainer.html`](output/case_study_evidence_explainer.html) |
...
| Tier 0（バリアント完全一致データ）の受入れ・検証・統合方法 | [`database/TIER0_INTEGRATION.md`](database/TIER0_INTEGRATION.md) |

パイプラインは[`run.sh`](run.sh)で実行可能（ダウンロード済みのデータや大容量の生成物はGitに含めず、配布元から再取得して作成します）。

## パイプラインから出力されるデータの概要

主要な評価対象は、総リンク数ではなく、**表現型連携によって研究で検討できるhuman-mouse variant pairを取得できるか**です。1:1 orthologを共通の足場にして、HPO-MP ontology、表現型名、共通疾患ID、表現型文のsemantic similarityを別々の接続根拠として保存します。単一の方法へ統一せず、同じvariant pairに複数のevidence rowを持たせる設計なので、複数回のペアが現れることもあります。一方で、一つのバリアントにつき、複数の遺伝子が該当するエリアについては、解消できていないのが現状です。

TLR4、CFTR、ALPLをcase studyにすると、指定した3組は接続方法が異なるものの、経路を統合した検索では3/3組を回収しました。これは、既知例が**今回の複数の接続手法で作成したバリアントペア情報から回収できるかを確認した結果**です。以下は、簡単なサマリーです。

| case study | human-mouse target pair | 主な回収経路 | 結果の意味 |
| --- | --- | --- | --- |
| TLR4 | rs4986790 - Tlr4<Lps-d> | ortholog + semantic class C | 厳密な3 Tierで落ちた対象ペアを、炎症性表現型の緩い候補として回収。LPS機序の一致を示すものではないところに注意 |
...
## Tier 0 human variant–mouse variant完全一致データ との統合について

human variant–mouse variant完全一致データは、phenotypeとは独立した**Tier 0 direct correspondence**として同じpair/evidenceモデルへ統合する設計にしています。ただし「完全一致」の意味を曖昧にしないため、`genomic_reciprocal_exact`、`coding_exact`、`protein_exact`を`match_level`で区別し、assembly、versioned transcript/alignment、strand、reciprocal mapping、元ファイルSHA-256を各行に保持します。
...
### ２０２６.9版追加: SapBERTを用いた、ortholog-first表現型文章類似度
...
| ファイル | 内容 |
| --- | --- |
| `run.sh` | 取得、変換、Tier/Semantic作成、検証を決めた順番で実行するパイプライン |
| `config/workflow.json` | 配布元URL、保存先、採用条件、Semantic閾値、Tier定義をrun.shに渡す configファイル |
| `data/external/tier0_exact_variant.template.tsv` | Tier 0を受け取るための列定義 |
...
| `output/methods_and_biological_evidence_en.html` | Methodsページの英語版 |
| `output/methods_data_linkage.svg` | human/mouse genomeからDBまでの連携手順を示す図 |
| `output/tier_matching_strategy_ja.svg` | 1:1 ortholog gene内でTier 1/2/3がhuman variantとmouse alleleを結ぶ日本語模式図 |
| `output/tier_matching_strategy_en.svg` | 上記のTier模式図の英語版 |
...
| --- | ---: | ---: | ---: | --- |
| TLR4 rs4986790 - Tlr4<Lps-d> | 0 | 0 | 0 | Tier側では未回収。別のsemantic class Cで回収 |
...
Monarchは、confidence 0.8以上以外にも緩い閾値のデータも含めていいかもしれませんが、今のところhigh confidenceで制作しています。
...
各工程を個別に調べるための従来の`npm run prepare`等も残していますが、再現には`run.sh`を利用してください。
